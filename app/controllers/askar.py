...
async def update_data(store_key, data_key, new_data):
    ...
    async with store.session() as session:
        await session.replace(
            "seq", data_key, json.dumps(new_data), {"~plaintag": "a", "enctag": "b"}
        )
...
async def update_oauth(store_key, oauth_data):
    store = await open_store(store_key)
    async for row in store.scan("seq", {"~plaintag": "a", "enctag": "b"}):
        if "oauth" in row.name and oauth_data["api_key_hash"] in row.value.decode(
            "utf-8"
        ):
            async with store.session() as session:
                await session.replace(
...
async def verify_client_hash(store_key, client_hash):
    ...
    async with store.session() as session:
        client_hashes = await session.fetch("seq", data_key)
...
async def verify_token(store_key, label, token):
    ...
    if oauth_data["access_token"] == token.replace("Bearer ", "") and oauth_data[
        "token_expiration"
    ] >= int(time.time()):
        return True
